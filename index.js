...
function complexConvert(data) {
  ...
  const jsonData = JSON.parse(data);  // Перетворюємо json дані в об'єкт та отримуємо всі ключі об'єкта.
   for (const key in jsonData) {
    if (typeof jsonData[key] === "number") { 
    // Обходимо всі ключі та перевіряємо значення.
    
      result[key] = jsonData[key] + 1; // Якщо значення є числом, збільшуємо його на 1.
    } else { 
      result[key] = jsonData[key]; // Якщо значення не є числом, просто копіюємо його у новий об'єкт без змін.
    }
   }
  return JSON.stringify(result); // Повертаємо оброблений об'єкт.
}
...
function manipulateUrl(url) {
  const parcedUrl = new URL(url); // Створюємо новий об'єкт URL.
  parcedUrl.protocol = 'https'; // Змінюємо протокол URL на https.
  parcedUrl.host = 'newhost.com';// Змінюємо хост URL на 'newhost.com'.
  parcedUrl.searchParams.append('newParam', 'newValue'); // Додаємо параметр 'newParam' зі значенням 'newValue' до URL.
  parcedUrl.searchParams.delete('oldParam'); // Видаляємо параметр 'oldParam' з URL, якщо він існує.
  const paramsArray = []; 
  ...
  return {
    href: parcedUrl.href, 
    protocol: parcedUrl.protocol, 
    host: parcedUrl.host,
    pathname: parcedUrl.pathname,
    search: parcedUrl.search, 
    params: paramsArray,
...
function searchParamsURL(url) {
  ...
  const searchParams = urlObject.searchParams;  // Отримання об'єкта URLSearchParams з властивості 'searchParams' об'єкта URL
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  const paramKeys = Object.keys(paramsObj); // Використовуючи метод 'keys' з об'єкта Object, отримуємо всі ключі paramsObj.
...
function deleteSearchParams(keys, url) {
  ...
  for (const key of keys) { 
    urlObj.searchParams.delete(key); // За допомогою циклу 'for of' перебираємо всі ключі та видаляємо відповідні параметри пошуку з urlObj.
  }
  return urlObj.toString(); // Повертаємо нову URL-адресу в рядковому форматі.
}
...
function createURLWithParams(params, url) {
  ...
  for (const key in params) { 
    urlObj.searchParams.set(key, params[key]); // За допомогою циклу 'for in' перебираємо всі ключі та значення об'єкта params та додаємо їх як параметри пошуку до urlObj.
  }
  return urlObj.toString(); // Повертаємо нову URL-адресу в рядковому форматі.
}
...
function modifyURLParameters(url, params) {
  const urlObj = new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
  for (const key in params) { 
    const value = params[key]; // Перебираємо словник params за допомогою for in
  // Отримаємо значення словника по ключу
  urlObj.searchParams.set(key, value); // Якщо параметр вже існує, метод set замінює його новим значенням.
  }
...
function checkURLParameters(url, params) {
  const urlObj = new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
  const result = {}; // Створюємо новий об'єкт для зберігання результатів.
  for (const param of params) { // Перебираємо елементи множини params за допомогою for of.
  ...
  return result; // Повертаємо об'єкт з результатами.
}
...
function processUrl(url, options) {
  ...
  if (options.searchParams) { // Перевіряємо, чи в об'єкті 'options' є параметри пошуку.

    for (const key in options.searchParams) { // Якщо є, перебираємо ці параметри за допомогою циклу 'for in'.
       urlObj.searchParams.append(key, options.searchParams[key]);  // Для кожного ключа параметру додаємо його та відповідне значення до об'єкта 'urlObj' за допомогою методу 'append'.
    }
  }
  if (options.protocol) { // Перевіряємо, чи в об'єкті 'options' є протокол.
...
function manipulateQuery(url, options) {
  const urlObj = new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
  if (options.has("append")) { // Якщо в словнику `options` є ключ `append`...
    const appendParams = options.get("append"); // ...перебираємо його ключі та значення за допомогою циклу for...of.
    for (const [key,value] of appendParams) { // Додаємо кожний ключ і значення до об'єкта `searchParams` в URL.
      urlObj.searchParams.append(key, value);
    }
  } 
  if (options.has("delete")) { 
    const deleteParams = options.get("delete"); // Якщо в словнику `options` є ключ `delete`...
...
function getURLValues(url) {
  ...
  const keysArray = Array.from(searchParams.keys()); // Отримуємо масив ключів пошукових параметрів.
  const valuesArray = []; // Масив для збереження значень пошукових параметрів.
  for (const key of keysArray) { // Перебираємо ключі пошукових параметрів.
    const values = searchParams.getAll(key); // Отримуємо всі значення для даного ключа за допомогою методу `getAll`.
    valuesArray.push(...values); // Додаємо значення до масиву.
  }
...
function getUrlKeys(url) {
  ...
  const keysArray = Array.from(searchParams.keys()); // Отримуємо масив зі всіма ключами пошукових параметрів за допомогою методу 'keys'.
  return keysArray; // Повертаємо масив з ключами.
}
